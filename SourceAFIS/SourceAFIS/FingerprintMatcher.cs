    ...
    public class FingerprintMatcher
    {
        ...
        private void SkipPaired()
        {
                                           ...
                                           || PairsByCandidate[PairQueue.Peek().Neighbor.Candidate] != null))
            {
                ...
                if (PairsByProbe[edge.Neighbor.Probe] != null && PairsByProbe[edge.Neighbor.Probe].Pair.Candidate ==
                    edge.Neighbor.Candidate)
                {
        ...
        private static List<MatchingPair> FindMatchingPairs(NeighborEdge[] probeStar, NeighborEdge[] candidateStar)
        {
            ...
            for (var candidateIndex = 0; candidateIndex < candidateStar.Length; ++candidateIndex)
            {
                var candidateEdge = candidateStar[candidateIndex];

                while (range.Begin < probeStar.Length && probeStar[range.Begin].Edge.Length <
                       candidateEdge.Edge.Length - MaxDistanceError)
                    ++range.Begin;
                if (range.End < range.Begin)
                    range.End = range.Begin;
                while (range.End < probeStar.Length && probeStar[range.End].Edge.Length <=
                       candidateEdge.Edge.Length + MaxDistanceError)
                    ++range.End;
                ...
                for (var probeIndex = range.Begin; probeIndex < range.End; ++probeIndex)
                {
                    ...
                    var referenceDiff = Angle.Difference(probeEdge.Edge.ReferenceAngle,
                        candidateEdge.Edge.ReferenceAngle);

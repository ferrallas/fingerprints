...
namespace SourceAFIS.Utils
{
    internal static class Angle
    {
        ...
        private static readonly double[] PrecomputedSin = PrecomputeSin();

        private static readonly double[] PrecomputedCos = PrecomputeCos();
        ...
        {
            var result = (int) (ToFraction(angle) * resolution);
            if (result < 0)
                return 0;

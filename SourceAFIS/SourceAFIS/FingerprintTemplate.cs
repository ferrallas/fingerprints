...
namespace SourceAFIS
{
    public class FingerprintTemplate
    {
            ...
            for (var blockY = 0; blockY < blocks.BlockCount.Y; ++blockY)
            {
                ...
                if (validMaskRange.Length > 0)
                {
                    ...
                    for (var y = blocks.BlockAreas[blockY, 0].Bottom; y < blocks.BlockAreas[blockY, 0].Top; ++y)
                        ...
                        {
                            var radius = Math.Max(Math.Abs(neighbor.CheckLocation.X),
                                Math.Abs(neighbor.CheckLocation.Y));
        ...
        private Point[][] ConstructOrientedLines(int resolution = 32, int radius = 7, double step = 1.5)
        {
        ...
        private static void RemoveCrosses(bool[,] input)
        {
            ...
            while (any)
            {
                ...
                for (var x = 0; x < size.X - 1; ++x)
                    if (input[y, x] && input[y + 1, x + 1] && !input[y + 1, x] && !input[y, x + 1] || input[y + 1, x] &&
                        input[y, x + 1] && !input[y, x] && !input[y + 1, x + 1])
                    {
                        input[y, x] = false;
                        input[y + 1, x] = false;
                        input[y, x + 1] = false;
        ...
        private void BuildEdgeTable()
        {
            ...
            for (var reference = 0; reference < EdgeTable.Length; ++reference)
            {
                ...
                for (var neighbor = 0; neighbor < Minutiae.Count; ++neighbor)
                    if (neighbor != reference && (referencePosition - Minutiae[neighbor].Position).SqLength <=
                        sqMaxDistance)
                    {

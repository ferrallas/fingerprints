    ...
    internal static class Delaunay2D
    {
        ...
        private struct DelaunayTriangulator
        {
            ...
            internal bool Initialize(ICollection<PointF> pointsCollection, float tolerance)
            {
                ...
                _tolerance = tolerance > 0 ? tolerance : float.Epsilon; // Ensure tolerance is valid

                _pointsCount = pointsCollection?.Count ?? 0;
                ...
                GetMinMaxPointCoordinates(_points, _pointsCount, out var pointsMin, out var pointsMax);
                ...
                var mid = new PointF
                {
                    X = (pointsMax.X + pointsMin.X) * 0.5f,
                    Y = (pointsMax.Y + pointsMin.Y) * 0.5f
                };
            ...
            private static void GetMinMaxPointCoordinates(IReadOnlyList<PointF> points, int count, out PointF min, out PointF max)
            {
                if (count <= 0)
            ...
            private static int[] GetSortedPointIndices(IReadOnlyList<PointF> points, int count, float tolerance)
            {

...
namespace PatternRecognition.FingerprintRecognition.Core.Medina2011
{
    ...
    /// </remarks>
    public class DelaunayMtpsProvider : ResourceProvider<MtripletsFeature>
    {
        #region public
        ...
        public override string GetSignature()
        {
            try
            {
                return string.Format("Delaunay({0}).mtp", MtiaListProvider.GetType().Name);
            }
            catch (Exception)
            {
                if (MtiaListProvider == null)
                    throw new InvalidOperationException("Unable to get signature of DelaunayMtpsProvider: Unassigned minutia list provider!");
                if (MtiaListProvider == null)
                    throw new InvalidOperationException("Unable to get signature of DelaunayMtpsProvider: Unassigned minutia list extractor!");
                throw;
            }
        }

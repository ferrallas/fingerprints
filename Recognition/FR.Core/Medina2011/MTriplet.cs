...
namespace PatternRecognition.FingerprintRecognition.Core.Medina2011
{
    ...
    internal class MTriplet
    {
        ...
        public MTriplet(Int16[] mIdxs, List<Minutia> minutiae)
        {
            ...
            for (byte i = 0; i < 2; i++)
                for (byte j = (byte)(i + 1); j < 3; j++)
                {
                    ...
                    double currBeta = Angle.DifferencePi(mtiaArr[i].Angle, mtiaArr[j].Angle);
        ...
        public short[] MtiaIdxs => mtiaIdxs;
        ...
        public double Match(MTriplet target, out byte[] mtiaOrder)
        {
            ...
            double maxSimil = 0;
            if (Math.Abs(MaxDistance - target.MaxDistance) < dThr && Math.Abs(MidDistance - target.MidDistance) < dThr && Math.Abs(MinDistance - target.MinDistance) < dThr)
        ...
        private double MatchMtiaDirections(MTriplet compareTo, byte[] order)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                Minutia qMtiai = minutiae[MtiaIdxs[i]];
                Minutia tMtiai = compareTo.minutiae[compareTo.MtiaIdxs[order[i]]];
                double diff = Angle.DifferencePi(qMtiai.Angle, tMtiai.Angle);
                if (diff >= Math.PI / 4)
                    return 0;
                if (diff > maxdiff)
                    maxdiff = diff;

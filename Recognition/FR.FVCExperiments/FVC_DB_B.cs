...
{
    /// <summary>
    ///     Abstract class containing an evaluation protocol for testing a fingerprint matching algorithm in databases DB1_B,
    ///     DB2_B, DB3_B and DB4_B from FVC2000, FVC2002 and FVC2004.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         The performance indicators computed in this experiments are: EER(%), FMR100(%), FMR1000(%), ZeroFMR(%),
    ///         Time(ms) and ROC curves. This indicators are saved in a file which name is formed by the name of the matching
    ///         algorithm concatenated with ".Summary.csv". This file is saved in a folder by the name of "Results" in the same
    ///         folder where the fingerprints are stored. Two more files are saved, one file containing the false matching
    ///         fingerprints and the other containing the false not matching fingerprints.
    ///     </para>
    ///     <para>
    ///         In order to execute the experiment, you must set the properties <see cref="Matcher" />,
    ///         <see cref="ResourceProvider" /> and <see cref="ResourcePath" />. Optionally, you can set the property
    ///         <see cref="StatusEvent" /> in order to get updates concerning the status of the experiment.
    ///     </para>
    /// </remarks>
    ...
    public abstract class FVC_DB_B : IMatchingExperiment
    {
        ...
        /// </summary>
        public event ProgressChangedEventHandler StatusEvent;

        /// <summary>
        ///     Executes the experiment.
        /// </summary>
        /// <exception cref="InvalidOperationException">
        ///     Thrown when the resource path is invalid, the matcher is not assigned or
        ///     the resource provider is not assigned.
        /// </exception>
        public void Execute()
        {
            VerifyPreconditions();
            ...
            var speed = DoComparisons(features, imgCaptions, out genuine, out impostor, out falseNotMatches,
                out falseMatches);
        ...
        private double DoComparisons(IList<object> features, List<string> imgCaptions, out List<double> genuine,
            out List<double> impostor, out List<string> falseNotMatches, out List<string> falseMatches)
        {
            genuine = new List<double>();
            impostor = new List<double>();
            var total = 28 * features.Count / 8 + 8 * (features.Count / 8) * (features.Count / 8 - 1) / 2;
            var percentDelta = total / 100;
            var idx = 0;
            var sw = new Stopwatch();
            sw.Start();
            // Performing genuine comparisons
            falseNotMatches = new List<string>();
            for (var i = 0; i < features.Count / 8; i++)
            for (var j = 0; j < 7; j++)
            for (var k = j + 1; k < 8; k++)
            {
                var s = Matcher.Match(features[i * 8 + j], features[i * 8 + k]);
                if (s == 0)
                {
                    var fingerprint1 = imgCaptions[i * 8 + j];
                    var fingerprint2 = imgCaptions[i * 8 + k];
                ...
                {
                    var progressPercent = Convert.ToInt32(Math.Round(1.0 * idx / total * 100));
            ...
            for (var k = 0; k < 8; k++)
            for (var i = 0; i < features.Count / 8 - 1; i++)
            for (var j = i + 1; j < features.Count / 8; j++)
            {
                var s = Matcher.Match(features[i * 8 + k], features[j * 8 + k]);
                if (s > 0)
                {
                    var fingerprint1 = imgCaptions[i * 8 + k];
                    var fingerprint2 = imgCaptions[j * 8 + k];
            ...
            GC.Collect();
            var ms = sw.Elapsed.Days * 24 * 3600 * 1000 + sw.Elapsed.Hours * 3600 * 1000 +
                     sw.Elapsed.Minutes * 60 * 1000 +
        ...
        private void ComputeAccuracy(List<double> genuine, List<double> impostor, out double eer, out double fmr100,
            out double fmr1000, out double zeroFmr, out List<ROCPoint> roc)
        {
            var rocBuilder = new FNMRvsFMR();
            roc = new List<ROCPoint>();
            foreach (var rocPoint in rocBuilder.BuildROC(genuine, impostor, new SimilarityValuesComparer()))
            ...
            double fnmr2 = double.NaN,
                fmr2 = double.NaN;
            zeroFmr = roc[0].y * 100;
            for (var i = 0; i < roc.Count; i++)
            {
                if (roc[i].x <= 0.001)
                    fmr1000 = roc[i].y * 100;
                else if (roc[i].x <= 0.01)
                    fmr100 = roc[i].y * 100;
                if (roc[i].y > roc[i].x)
                {
                    fnmr2 = roc[i + 1].y;
        ...
        private List<object> GetFeatures(out List<string> imgCaptions)
        {
            ...
            var features = new List<object>(80);
            ...
            for (var j = 1; j <= 8; j++)
            {
                ...
                if (StatusEvent != null)
                {
                    var progressPercent = Convert.ToInt32(Math.Round(1.0 * idx / 80 * 100));
